...
USUARIOS_PERMITIDOS = {
    ...
    "Salsicha": "AVRZ123", 
    "Sauer": "admin123",
    "Teste": "teste00",          # Senha atualizada para teste00
    "Enzo": "enzoflamengo123",    # Novo Usuário
    "Natan Sauer": "ns1234",      # Novo Usuário
    "Pedro Sauer": "ps5678"       # Novo Usuário
}
...
def criar_tabela():
    try:
        ...
        try: cur.execute('ALTER TABLE palpites ADD COLUMN racha_desafio VARCHAR(50)')
        except: conn.rollback()
        ...
        try: cur.execute('ALTER TABLE jogos_admin ADD COLUMN status_manual VARCHAR(20)')
        except: conn.rollback()
...
# --- MOTOR DE CÁLCULO DE PONTOS, ENGENHARIA FINANCEIRA E BINGO ---
def processar_ranking_e_financas():
    ...
    for p in todos_palpites:
        j_id = p[2]
        if j_id not in [] : palpites_por_jogo.setdefault(j_id, []).append(p)
    ...
    bingos_jogos = {} # {jogo_id: [lista_usuarios]}
    
    # Processar jogo por jogo de maneira cirúrgica
    for j_id, palpites in palpites_por_jogo.items():
        ...
        # Coleta os resultados reais da partida
        g_a_real = jogo['gols_a_real'] if jogo else 'N/A'
        g_b_real = jogo['gols_b_real'] if jogo else 'N/A'
        
        # Mapeamento de classes por jogo para a Regra de 50 centavos (Ideia 4)
        ...
        # Primeiro passo do laudo: ver quem participou de cada classe
        for p in palpites:
        ...
        # Segundo passo do laudo: Se jogo encerrado, validar quem acertou cada classe
        if status_final == 'ENCERRADO' and g_a_real != 'N/A' and g_b_real != 'N/A':
            ga_r, gb_r = int(g_a_real), int(g_b_real)
            
            # Checar ganhadores por classe
            ...
            # Terceiro passo: Calcular Finanças baseadas estritamente na IDEIA 4 e Pontos
            pontos_jogo_usuario = {}
            ...
            for p in palpites:
                ...
                pts_ganhos = 0
                is_bingo = True # Inicia como verdadeiro, se errar qualquer coisa vira falso
                
                # Cálculo de pontos do Placar
            ...
            # Processar os Rachas 1v1 (Ideia 9 - Dobro de pontos)
            for user, p_data in pontos_jogo_usuario.items():
                ...
                if racha_alvo in pontos_jogo_usuario:
                    # Se o desafiante fez estritamente mais pontos que o adversário, ganha o dobro!
                    if p_data['pts'] > pontos_jogo_usuario[racha_alvo]['pts']:
                        final_pts *= 2
                ...
                if p_data['is_bingo']:
                    bingos_jogos.setdefault(j_id, []).append(user)

            # Computar cobrança financeira real da Ideia 4
            for cl, lista_p in classes_apostadores.items():
                if len(lista_p) >= 2 and classes_ganhadores[cl]:
                    # Todos que apostaram nessa classe gastam 50 centavos
                    for p in lista_p:
                        u = p[1]
...
def index():
    ...
    # Verificar quais jogos o usuário logado já palpitou (Ideia 1)
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("SELECT jogo_id FROM palpites WHERE usuario = %s", (user_logado,))
    jogos_palpitados = set([r[0] for r in cur.fetchall()])
    cur.close(); conn.close()
    
    # Injetar o status do palpite e os bingos nos cards
    for jogo in jogos:
        jogo['ja_palpitado'] = jogo['id'] in jogos_palpitados
        jogo['bingo_vencedores'] = bingos.get(jogo['id'], [])
        
    return render_template('index.html', usuario=user_logado, jogos=jogos, gasto=f"{total_pool:,.2f}".replace('.', ','), usuarios_lista=USUARIOS_PERMITIDOS.keys())
...
def perfil():
    ...
    # Calcular Taxa de Sucesso baseada em palpites válidos
    ...
    # --- SISTEMA DE CONQUISTAS DIVERTIDAS DO FUTEBOL (Ideia 6) ---
    conquistas = []
    if user_stats['maior_erro'] >= 5:
        conquistas.append({"nome": "🦶 PÉ FRIO / BOCA PODRE", "desc": "Errou um placar por uma diferença monumental de 5 ou mais gols. Calibra essa luneta!"})
    if user_stats['cravadas'] >= 3:
        conquistas.append({"nome": "🔮 MÃE DINÁH DA ARENA", "desc": "Cravou 3 ou mais placares cheios na mosca. Visão de águia!"})
    if user_stats['artilheiros_certos'] >= 2:
        conquistas.append({"nome": "⚽ CHEIRA-GOL", "desc": "Acertou o artilheiro oficial da partida por 2 ou mais vezes."})
    if user_stats['pontos'] >= 30:
        conquistas.append({"nome": "👑 CAMISA 10 DA RESENHA", "desc": "Passou a marca histórica dos 30 pontos globais no bolão."})
        
    if not conquistas:
        conquistas.append({"nome": "🏃 NO AQUECIMENTO", "desc": "Ainda pegando o ritmo de jogo na pista. Seus troféus vão aparecer aqui!"})
...
def info_torneio():
    ...
    stats, _, _ = processar_ranking_e_financas()
    # Criar Leaderboard ordenado por pontos para a aba Ranking (Ideia 3)
