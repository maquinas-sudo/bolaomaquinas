...
def index():
    ...
    for jogo in jogos:
        adm = admin_data.get(jogo['id'], {})
        
        # --- CÂMBIO INTELIGENTE ---
        # Se você marcou como AO VIVO, mas a API mandou ENCERRADO, o sistema respeita a API e trava o jogo!
        if adm.get('status_manual'):
            if jogo['status'] == 'ENCERRADO' and adm['status_manual'] == 'AO VIVO':
                pass # Ignora o "AO VIVO" manual porque a API já decretou o fim
            else:
                jogo['status'] = adm['status_manual']
...
def perfil():
    ...
    cur.execute("SELECT jogo_id, status_manual FROM jogos_admin")
    admin_status = {r[0]: r[1] for r in cur.fetchall()}
    cur.close(); conn.close()
    ...
    for a in apostas:
        ...
        if jogo:
            status_api = jogo['status']
            status_manual = admin_status.get(j_id)
            
            # Aplica o mesmo Câmbio Inteligente na tela de pontuação
            status_final = status_api
            if status_manual:
                if status_api == 'ENCERRADO' and status_manual == 'AO VIVO':
                    status_final = 'ENCERRADO'
                else:
                    status_final = status_manual

            if status_final == 'ENCERRADO' and jogo['gols_a_real'] != 'N/A':
                try:
...
def apostar():
    ...
    if len(artilheiro) > 20: return jsonify({"sucesso": False, "erro": "Artilheiro muito longo!"}), 400
